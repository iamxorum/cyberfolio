    ...
    <>
      ...
      <div className="layout-content-container flex flex-col max-w-[1200px] w-full flex-1">
        <header className="flex items-center justify-between whitespace-nowrap px-3 sm:px-4 md:px-6 lg:px-10 py-2 sm:py-3">
          <Link href="/" className="flex items-center gap-2 sm:gap-4 text-[var(--terminal-text)] min-w-0 hover:opacity-80 active:scale-[0.97] transition-all">
            ...
            <nav className="flex items-center gap-2 sm:gap-3 md:gap-4 lg:gap-6 xl:gap-9" aria-label="Primary">
              {navItems.map((item) => (
                <Link
                  ...
                  className={`transition-colors active:scale-[0.97] inline-block text-[10px] sm:text-xs md:text-sm font-medium leading-normal font-mono whitespace-nowrap ${
                    pathname === item.href
